...
public final class CheckTypeMenu {
    ...
    public static void open(Player player, String category) {
        GuiHolder holder = new GuiHolder(GuiHolder.MenuType.CHECK_TYPES, null, null, category, 0);
        Inventory inv = Bukkit.createInventory(holder, 45, getTitle(category));
        ...
        for (CheckType type : types) {
            ...
            lore.add(GuiConstants.DARK + GuiConstants.getTypeDescription(type));
            lore.add("");
            lore.add(GuiConstants.DARK + GuiConstants.SYM_LINE + " " + GuiConstants.MUTED + "Checks  " + (allOn ? GuiConstants.SUCCESS : enabled > 0 ? GuiConstants.SECONDARY : GuiConstants.ERROR) + enabled + GuiConstants.DARK + "/" + GuiConstants.MUTED + total);
            ...
            ItemStack item = allOn
                    ? GuiItemFactory.createGlowing(GuiConstants.getIcon(type), GuiConstants.HIGHLIGHT + GuiConstants.BOLD + type.getName(), lore)
                    : GuiItemFactory.create(GuiConstants.getIcon(type), GuiConstants.MUTED + GuiConstants.BOLD + type.getName(), lore);
